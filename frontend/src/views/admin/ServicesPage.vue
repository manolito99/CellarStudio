<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-[#2B2E2E]">Servicios</h1>
      <button
        @click="openModal()"
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="service in services" :key="service.id" class="hover:bg-gray-50">
            <td class="px-4 py-3">
              <p class="text-[#2B2E2E] font-medium">{{ service.name }}</p>
            ...
            <td class="px-4 py-3 text-[#2B2E2E]">{{ service.duration_minutes }} min</td>
            <td class="px-4 py-3">
              <button
                @click="toggleActive(service)"
                class="px-3 py-1 rounded-full text-xs font-medium transition-colors"
                :class="service.is_active ? 'bg-green-500/10 text-green-400' : 'bg-gray-100 text-[#595959]'"
              >
            ...
            <td class="px-4 py-3">
              <div class="flex gap-1">
                <button @click="openModal(service)" class="p-1.5 rounded-lg hover:bg-gray-100 text-[#595959] hover:text-[#2B2E2E] transition-colors">
      ...
      <div class="bg-white border border-gray-200 rounded-2xl w-full max-w-lg p-6">
        ...
        <form @submit.prevent="saveService" class="space-y-4">
          ...
          <div>
            ...
            <textarea v-model="form.description" rows="2" class="w-full px-3 py-2 bg-[#F2F0E9] border border-gray-200 rounded-lg text-[#2B2E2E] focus:border-brand-400 focus:outline-none resize-none" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm text-dark-300 mb-1">Precio ($)</label>
              <input v-model.number="form.price" type="number" step="0.01" required class="w-full px-3 py-2 bg-[#F2F0E9] border border-gray-200 rounded-lg text-[#2B2E2E] focus:border-brand-400 focus:outline-none" />
            </div>
            <div>
              <label class="block text-sm text-dark-300 mb-1">Duración (min)</label>
              <input v-model.number="form.duration_minutes" type="number" required class="w-full px-3 py-2 bg-[#F2F0E9] border border-gray-200 rounded-lg text-[#2B2E2E] focus:border-brand-400 focus:outline-none" />
            </div>
          </div>
